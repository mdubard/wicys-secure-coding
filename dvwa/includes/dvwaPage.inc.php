...
$security_levels = array('v1', 'v2', 'v3', 'v4');
...
function dvwaHtmlEcho( $pPage ) {
	...
	if( dvwaIsLoggedIn() ) {
		$menuBlocks[ 'features' ] = array();
		$menuBlocks[ 'features' ][] = array( 'id' => 'login', 'name' => 'Login', 'url' => 'features/login/' );
		$menuBlocks[ 'features' ][] = array( 'id' => 'ping', 'name' => 'Ping any IP', 'url' => 'features/ping/' );
		$menuBlocks[ 'features' ][] = array( 'id' => 'upload', 'name' => 'File Upload', 'url' => 'features/upload/' );
		$menuBlocks[ 'features' ][] = array( 'id' => 'captcha', 'name' => 'CAPTCHA', 'url' => 'features/captcha/' );
		$menuBlocks[ 'features' ][] = array( 'id' => 'change_id_view', 'name' => 'Change ID and View Changes', 'url' => 'features/change_id_view/' );
		$menuBlocks[ 'features' ][] = array( 'id' => 'change_id_blind', 'name' => 'Change ID', 'url' => 'features/change_id_blind/' );
		$menuBlocks[ 'features' ][] = array( 'id' => 'gen_sess_ids', 'name' => 'Generate Session IDs', 'url' => 'features/gen_sess_ids/' );
		$menuBlocks[ 'features' ][] = array( 'id' => 'name_entry', 'name' => 'Enter Name', 'url' => 'features/name_entry/' );
		$menuBlocks[ 'features' ][] = array( 'id' => 'sign_guestbook', 'name' => 'Sign Guestbook', 'url' => 'features/sign_guestbook/' );
		$menuBlocks[ 'features' ][] = array( 'id' => 'javascript', 'name' => 'JavaScript', 'url' => 'features/javascript/' );
		if (dvwaCurrentUser() == "admin") {
			$menuBlocks[ 'features' ][] = array( 'id' => 'adminportal', 'name' => 'Admin Portal', 'url' => 'features/adminportal/' );
		}
	}
	...
	switch( dvwaSecurityLevelGet() ) {
		case 'v1':
			$securityLevelHtml = 'v1';
			break;
		case 'v2':
			$securityLevelHtml = 'v2';
			break;
		case 'v3':
			$securityLevelHtml = 'v3';
			break;
		default:
			$securityLevelHtml = 'v4';
			break;
	}
	// -- END (security cookie)
...
function dvwaButtonHelpHtmlGet( $pId ) {
	...
	return "<input type=\"button\" value=\"View Help\" class=\"popup_button\" id='help_button' data-help-url='" . DVWA_WEB_PAGE_TO_ROOT . "features/view_help.php?id={$pId}&security={$security}&locale={$locale}' )\">";
}


function dvwaButtonSourceHtmlGet( $pId ) {
	$security = dvwaSecurityLevelGet();
	return "<input type=\"button\" value=\"View Source\" class=\"popup_button\" id='source_button' data-source-url='" . DVWA_WEB_PAGE_TO_ROOT . "features/view_source.php?id={$pId}&security={$security}' )\">";
}
